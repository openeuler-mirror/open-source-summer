...
def main(cfg):
    ...
    es_loss, es_f1, es_epoch, es_patience, best_es_epoch, best_es_f1 = 1e8, -1, 0, 0, 0, -1
    es_model = model
    best_es_model = model
    flag = False
    ...
    for epoch in range(1, cfg.epoch + 1):
        ...
        if es_loss > valid_loss:
            ...
            if es_patience >= cfg.early_stopping_patience:
                ...
                best_es_f1 = es_f1
                best_es_model = es_model
                flag = True

    #保存训练模型
    if flag:
        torch.save(best_es_model.state_dict(), os.path.join(utils.get_original_cwd(), cfg.output_dir, cfg.model_save_name))
    else:
        torch.save(model.state_dict(), os.path.join(utils.get_original_cwd(), cfg.output_dir, cfg.model_save_name))
